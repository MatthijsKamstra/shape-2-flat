...
- Glue tabs are 7 mm; vertical seams use saw-tooth triangles when the segment `type` is `arc` or `curve`, otherwise 45° miters, and fold lines render in `FOLDING_LINES` as white dashed `stroke-dasharray="2,1"`.
