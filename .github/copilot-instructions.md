...
- Input comes from `--input` SVG content or `--path` data; outputs SVG with groups `GLUE_SIDE`, `GLUE_SHAPE`, `SHAPE`, `FOLDING_SIDE`, `FOLDING_SHAPE`, `CUT_LINES`, `DESIGN`, `INFO`, `BG` plus meta `{faces, perimeter, area}`.
