...
describe("Shape Glue Tabs - Rectangle", () => {
	...
	it("should have 7mm tab height for horizontal edges", () => {
		...
		// Check that tabs have proper dimensions
		assert.ok(glueShapeGroup.length > 0, "Should have glue tabs");

		// Verify tab geometry by checking that we have trapezoid paths with appropriate offsets
		// Tabs should extend ~7mm from the edge, creating coordinates with calculated offsets
		const pathMatches = glueShapeGroup.match(/<path d="[^"]+"/g);
		assert.ok(pathMatches && pathMatches.length >= 8, "Should have at least 8 tab paths (2 per edge)");

		// Check that paths contain trapezoid structure (M L L L Z pattern)
		const hasTrapezoids = pathMatches.some(p => /M [^L]+ L [^L]+ L [^L]+ L [^Z]+ Z/.test(p));
		assert.ok(hasTrapezoids, "Should have trapezoid-shaped tabs");
	});
