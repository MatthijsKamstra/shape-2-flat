...
// Use Node packages only when running under Node (CLI). In the browser use native DOMParser and SVG APIs.
let DOMParserNode = null;
let xpathNode = null;
let SvgPathFactory = null;
if (typeof window === "undefined") {
	const { createRequire } = await import("module");
	const requireNode = createRequire(import.meta.url);
	try {
		const xmldom = requireNode("@xmldom/xmldom");
		DOMParserNode = xmldom.DOMParser;
	} catch { }
	try {
		xpathNode = requireNode("xpath");
	} catch { }
	try {
		const svgpathModule = requireNode("svgpath");
		SvgPathFactory = svgpathModule?.default || svgpathModule;
	} catch { }
}
...
function applyCtmToPathD(d, node) {
	...
	// If svgpath is available (Node), use it; otherwise skip transform in browser fallback.
	try {
		if (SvgPathFactory) return SvgPathFactory(d).transform(`matrix(${M.join(" ")})`).toString();
	} catch { }
	return d;
}
...
function extractPathInfo(svgContent) {
	...
	try {
		// Parse using browser DOMParser when available, otherwise use Node DOMParser from xmldom
		let doc;
		if (typeof window !== 'undefined') {
			doc = new DOMParser().parseFromString(svgContent, "image/svg+xml");
		} else if (DOMParserNode) {
			doc = new DOMParserNode().parseFromString(svgContent, "text/xml");
		} else {
			return { d: null, kind: null };
		}

		// Prefer xpath if available (Node), otherwise use DOM methods
		const select = xpathNode && xpathNode.useNamespaces ? xpathNode.useNamespaces({ svg: "http://www.w3.org/2000/svg" }) : null;
		// prefer explicit path
		let node = null;
		if (select) node = select("//svg:path[1]", doc)[0];
		if (!node && doc.getElementsByTagName) node = doc.getElementsByTagName('path')[0];
		...
		// rect
		node = select ? select("//svg:rect[1]", doc)[0] : (doc.getElementsByTagName ? doc.getElementsByTagName('rect')[0] : null);
		if (node) {
		...
		// circle
		node = select ? select("//svg:circle[1]", doc)[0] : (doc.getElementsByTagName ? doc.getElementsByTagName('circle')[0] : null);
		if (node) {
		...
		// ellipse
		node = select ? select("//svg:ellipse[1]", doc)[0] : (doc.getElementsByTagName ? doc.getElementsByTagName('ellipse')[0] : null);
		if (node) {
			const cx = parseFloat(node.getAttribute("cx") || 0);
			const cy = parseFloat(node.getAttribute("cy") || 0);
		...
		// polygon/polyline
		let polyNode = null;
		if (select) polyNode = select("//svg:polygon[1]", doc)[0] || select("//svg:polyline[1]", doc)[0];
		if (!polyNode && doc.getElementsByTagName) polyNode = doc.getElementsByTagName('polygon')[0] || doc.getElementsByTagName('polyline')[0];
		node = polyNode;
		if (node) {
...
export { extractPathD, extractPathInfo };
